...
class NeuralNet:
    def __init__(self, num_inputs, num_hidden_layers, num_hidden_neurons,num_output_neurons,alpha):
...
##        pdb.set_trace()
    ...
    def BackPropagate(self, predicted_output, target_output):
        deltas = []
        ##pdb.set_trace()
        momentum = 0.1
        for k in reversed(range(1, len(self.layers))):
            ...
            for j, n_j in enumerate(self.layers[k].neurons):
                dw = []
                if n_j.neuron_type == NeuronType.bias:
                    continue
                
                if self.layers[k].layer_type == LayerType.output_layer:
                    o_j = predicted_output[j]
                    t_j = target_output[j]
                    delta_j = (o_j - t_j) * n_j.derivative
                else:
                    if self.layers[k+1].layer_type == LayerType.output_layer:
                        s = sum([next_layer_deltas[l] * n_l.weights[j] for l,n_l in enumerate(self.layers[k+1].neurons)])
                    else:
                        s = sum([next_layer_deltas[l] * n_l.weights[j] for l,n_l in enumerate(self.layers[k+1].neurons[1:],1)])
                        
                    delta_j = s * n_j.derivative
                
                deltas.append(delta_j)
                
                for i, n_i in enumerate(self.layers[k-1].neurons):
                    o_i = self.outputs[k-1][i]
                    dw.append(-self.alpha * delta_j * o_i * momentum)
##                pdb.set_trace()
                n_j.UpdateWeights(dw)
                momentum = 1# .1 * sum(dw)
...
##            
        ...
        self.BackPropagate(predicted_output, target_output)
    ...
    def Train(self, training_examples, actual_outputs):
        ...
        self.errors = []
        self.vals = []
        while i == 0 or True:# or abs(rmse - prev_rmse) > 0.000001:
            errors = []
    ...
    def CrossValidate(self,examples,outputs,folds=10):
        n = len(examples)
        for i in range(folds):
            training_examples = examples[0:i*n/folds]+examples[(i+1)*n/folds:n]
            training_outputs = outputs[0:i*n/folds]+examples[(i+1)*n/folds:n]
            testing_examples = examples[i*n/folds:(i+1)*n/folds]
            testing_outputs = outputs[i*n/folds:(i+1)*n/folds]
            self.acc = 0
            self.prev_acc = float('inf')
            while abs(self.acc - self.prev_ass) > .005:
                self.Train(training_examples,training_outputs)
                self.acc = self.Test(testing_examples,testing_outputs)

def Test():
    global N,x,S
    x = [[-1,100,-20.5],[1,2,3,4],[20,-10,40,1]]
    S = [[1,1],[0,0],[1,1]]
    alpha = .1
    N = NeuralNet(len(x[0]),1,20,len(S[0]),alpha)
    N.Train(x,S)
